...
DatabasAPI.query(new DatabaseStatement("SELECT * FROM `my_table` WHERE kills > ?", 10, resultSet -> {
  ...
  // This code will be executed, when Client didn't get query response in selected time (default 5s)
...
## Known bugs
 - [Not confirmed] After few sector changes entities near border (both sides) clone infinity
